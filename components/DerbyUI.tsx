...
import { SOLO_RIVALS, soloName, type LocalMode } from '@/game/solo';
import courseLayout from '@/game/course-layout.json';

export interface DerbyUIProps {
  mode: LocalMode; onModeChange: (mode: LocalMode) => void; onPause: () => void; onSteer: (value: number) => void;
...
export function GarageCard({ player, build, onChange, controllerDriven = false, solo = false }: { solo?: boolean; player: PlayerId; build: Blueprint; onChange: (blueprint: Blueprint) => void; controllerDriven?: boolean }) {
  ...
  return <section className={`garage-card player-${player}`} style={{ '--player-color': PLAYER_COLORS[player] } as CSSProperties} aria-label={`Player ${player + 1} garage`}>
    ...
    <div className="garage-card-foot"><kbd>{controllerDriven ? `P${player + 1}` : solo ? 'SPACE' : PLAYER_KEYS[player]}</kbd><span>{controllerDriven ? 'Choose your ride on your phone' : 'Hold to charge. Release to hop.'}</span></div>
  </section>;
}
...
function SoloRaceHUD({ snapshots, onHold, onCancelInput, onSteer, countdown }: { onSteer: (value: number) => void; snapshots: VehicleSnapshot[]; onHold: (held: boolean) => void; onCancelInput?: () => void; countdown: boolean }) {
  const player = snapshots.find(snapshot => snapshot.id === 0);
  ...
  const sector = (courseLayout.sectors.find(sector=>distance<sector.endS)?.name || 'Pier Pressure').toUpperCase();
...
export default function DerbyUI(props: DerbyUIProps) {
  ...
  return <div className={`derby-ui stage-${stage} ${landing ? 'is-landing' : ''} ${props.phoneRoom ? 'phone-mode' : ''} ${solo ? 'solo-mode' : ''}`}>
    <header className="derby-header"><button className="brand-home" aria-label="Silicon Racer home" onClick={() => { if (stage === 'garage' && !props.phoneRoom) setEntered(false); }}><SiliconBrand compact/></button><span className="event-chip">{props.phoneRoom ? `ROOM ${props.phoneRoom}` : solo ? 'SOLO GRAND PRIX · SAN FRANCISCO' : 'SAN FRANCISCO SOAPBOX CLUB'}</span><div className="derby-header-actions">{!landing && props.onPhoneParty && <Button variant="outline" className="phone-party-button" onClick={props.onPhoneParty}><Smartphone size={16}/>{props.phoneRoom ? 'Invite friends' : 'Play with phones'}</Button>}<button className="icon-button" onClick={props.onToggleSound} aria-label={props.muted ? 'Turn sound on' : 'Mute sound'}>{props.muted ? <VolumeX size={20}/> : <Volume2 size={20}/>}</button><button className="icon-button" onClick={() => setShowRules(!showRules)} aria-label="How to play" aria-expanded={showRules}><HelpCircle size={20}/></button></div></header>
    {showRules && <aside className="rules-popover"><h2>{solo ? "Build. Steer. Send it." : "One button. All the glory."}</h2><p>Pick a chassis and wheels. Gravity does the driving.</p><p><strong>Hold to charge. Release to hop.</strong><br/>{props.phoneRoom ? 'Use the big button on your phone.' : solo ? 'Arrow keys steer. Hold Space or F to charge; release to hop. Or use the on-screen controls. Escape pauses.' : 'On a keyboard: player 1 uses F, player 2 uses J.'}</p><p>Three heats. Finish ahead to earn more points: 5, 3, 2, 1. Ties share the points.</p><Button className="small-action" onClick={() => setShowRules(false)}>Got it <Check size={17}/></Button></aside>}
    {landing && <section className="landing-screen"><div className="landing-copy"><SiliconBrand/><span className="solo-edition"><Bot size={15}/> BAY OR BUST · SOLO GRAND PRIX</span><h1>Big ideas. Bad brakes.</h1><p>Race three rivals through San Francisco.<br/>Build it. Steer it. Send it.</p><Button className="start-button landing-start" disabled={!loaded} onClick={() => { props.onModeChange('solo'); setEntered(true); }}>{loaded ? 'Race solo' : 'Loading Bay or Bust…'} <ArrowRight size={26}/></Button><div className="landing-multiplayer"><button onClick={props.onPhoneParty}>Host friends <Smartphone size={15}/></button><Link href="/play">Join a race</Link></div><button className="keyboard-link" disabled={!loaded} onClick={() => { props.onModeChange('local'); setEntered(true); }}>2 players on this keyboard</button></div><div className="landing-world"><LandingScene/><div className="landing-caption"><span className="live-dot"/> BUILT IN SF. BARELY STREET LEGAL.</div></div><div className="landing-footnote">1 player + 3 AI rivals <span>•</span> Arrow keys + Space <span>•</span> No downloads</div></section>}
    {stage === 'garage' && !landing && (props.phoneRoom ? <section className="lobby-screen"><div className="screen-heading"><span className="eyebrow">HEAT {heat} OF 3</span><h1>Grab your phone.</h1><p>Scan. Pick a racer. Get ready.</p></div><div className="lobby-content"><PartyJoinCode code={props.phoneRoom}/><div className="lobby-crew"><PartySeats players={props.partyPlayers} builds={builds} ready={props.phoneReady}/><output className="lobby-status"><span className="live-dot"/>{racerIds.length < 2 ? 'Waiting for at least 2 racers' : readyCount === racerIds.length ? 'Everyone’s ready. Here we go!' : `${readyCount} of ${racerIds.length} ready`}</output></div></div><p className="lobby-note">The race starts when everyone is ready. Keep this screen open to watch.</p></section> : <section className={`local-garage ${solo ? 'solo-garage' : ''}`}><div className="screen-heading"><button className="text-button" onClick={() => setEntered(false)}><ArrowLeft size={16}/> Back</button><h1>{solo ? "Built for the downhill." : "Pick your racer."}</h1><p>{solo ? "Your ride. Your rules. A very questionable idea." : "Choose a chassis. Add wheels. Send it."}</p></div><div className="garage-layout">{(solo ? [0] as PlayerId[] : [0, 1] as PlayerId[]).map(player => builds[player] && <GarageCard solo={solo} key={player} player={player} build={builds[player]} onChange={build => onBuildChange(player, build)}/>)}{solo && <div className="solo-showroom"><div className="showroom-title"><span className="eyebrow">YOUR CONTENDER · NO. 01</span><h2>{getBody(builds[0].bodyId).name}</h2><p>{getBody(builds[0].bodyId).description}</p></div><div className="solo-mobile-preview"><BodyGlyph id={builds[0].bodyId} color={PLAYER_COLORS[0]}/></div><div className="solo-rivals"><div className="rivals-heading"><span>MEET YOUR COMPETITION</span><span><Bot size={14}/> 3 AI DRIVERS</span></div>{SOLO_RIVALS.map(rival => <div className="rival-preview" key={rival.id}><span className="rival-dot" style={{ background: PLAYER_COLORS[rival.id] }}/><BodyGlyph id={rival.build.bodyId} color={PLAYER_COLORS[rival.id]}/><div><strong>{rival.name}</strong><span>{rival.style}</span></div></div>)}</div></div>}</div><footer className="garage-footer"><span>HEAT {heat} / 3</span><Button className="start-button" disabled={!canRace} onClick={onStart}>{loaded ? solo ? 'Let’s race' : 'Race!' : 'Loading the hill…'}<ArrowRight size={23}/></Button><span>{solo ? <><kbd>←</kbd> <kbd>→</kbd> steer · <kbd>SPACE</kbd> hop</> : <><kbd>F</kbd> + <kbd>J</kbd> to hop</>}</span></footer></section>)}
    {race && <><div className="race-topline"><span>HEAT {heat} / 3</span><strong className={props.finishCountdown != null ? 'finish-clock' : ''}>{props.finishCountdown != null ? `FINISH IN ${Math.ceil(props.finishCountdown)}s` : `${elapsed.toFixed(1)}s`}</strong>{!props.phoneRoom && <><button className="icon-button" onClick={props.onPause} aria-label="Pause race"><Pause size={16}/></button><button className="icon-button" onClick={event=>{event.currentTarget.blur();onReset();}} aria-label="Restart heat"><RotateCcw size={16}/></button></>}</div>{solo ? <SoloRaceHUD snapshots={snapshots} onHold={held => onHold(0, held)} onCancelInput={() => onCancelInput?.(0)} onSteer={props.onSteer} countdown={stage === 'countdown'}/> : <div className={`race-lanes ${active.length > 2 ? 'four-lanes' : ''}`}>{active.map(player => <RaceLane key={player} player={player} snapshot={snapshots.find(s => s.id === player)} snapshots={snapshots} spectator={!!props.phoneRoom} onHold={held => onHold(player, held)} onCancelInput={onCancelInput} countdown={stage === 'countdown'}/>)}</div>}{stage === 'countdown' && <div className="countdown-overlay"><span>SEE YOU AT THE BOTTOM.</span><strong key={countdown}>{countdown > 0 ? countdown : 'GO!'}</strong><p>Wait for GO. Hold to charge. Release to hop.</p></div>}</>}
    {results && <div className="results-backdrop"><section className="results-card"><span className="eyebrow">{final ? 'THE BRAGGING RIGHTS ARE IN' : `HEAT ${heat} COMPLETE`}</span><div className="results-headline">{final ? <Trophy size={38}/> : <Flag size={32}/>}<h1>{final ? winners.length > 1 ? 'Shared glory!' : solo && winners[0] === 0 ? 'The hill is yours!' : `${name(winners[0])} wins!` : solo && ranked[0] === 0 ? 'You sent it.' : 'What a ride.'}</h1><p>{final ? 'Big ideas. Extremely questionable driving.' : 'One hill closer to glory.'}</p></div>{solo && <div className="podium-spotlight"><BodyGlyph id={builds[ranked[0] ?? 0]?.bodyId || 'sourdough'} color={PLAYER_COLORS[ranked[0] ?? 0]}/><span>{name(ranked[0] ?? 0)}<small>{final ? 'GRAND PRIX WINNER' : 'HEAT WINNER'}</small></span></div>}<div className="results-players">{ranked.map(player => { const snapshot = snapshots.find(s => s.id === player); const place = final ? 1 + active.filter(id => (scores[id] || 0) > (scores[player] || 0)).length : snapshot ? racePlace(snapshot, snapshots) : '—'; return <article className={`result-player ${solo && player === 0 ? 'is-you' : ''}`} key={player} style={{ '--player-color': PLAYER_COLORS[player] } as CSSProperties}><span className="result-place">{place}</span><BodyGlyph id={builds[player]?.bodyId || 'sourdough'} color={PLAYER_COLORS[player]}/><div className="result-name"><strong>{name(player)}{solo && <small className="driver-type">{player === 0 ? "YOU" : "AI"}</small>}</strong><span>{snapshot?.finished ? timeLabel(snapshot.finishTime) : `${Math.round((snapshot?.progress || 0) * 100)}% of the hill`}{!final && ` · +${pointsLabel(points[player])} pts`}</span></div><div className="standing-score">{pointsLabel(scores[player])}<small>PTS</small></div></article>; })}</div><footer className="results-footer">{props.phoneRoom ? <output>{final ? 'Ready up on your phones for a rematch.' : heat < 3 ? 'Your garages open in a moment…' : 'The final podium is coming…'}</output> : <Button className="start-button" onClick={final ? onRematch : onNext}>{final ? 'Play again' : heat < 3 ? 'Next heat' : 'See the podium'}<ArrowRight size={23}/></Button>}</footer></section></div>}
  </div>;
}
