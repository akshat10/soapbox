...
function SoloRaceHUD({ onBoost, snapshots, onHold, onCancelInput, onSteer, countdown, moment, elapsed }: { onBoost: () => void; moment: SoloMoment | null; elapsed: number; onSteer: (value: number) => void; snapshots: VehicleSnapshot[]; onHold: (held: boolean) => void; onCancelInput?: () => void; countdown: boolean }) {
  ...
  const boostCharge = player?.boostCharge ?? 1;
  const boosting = (player?.boostRemaining ?? 0) > 0;
  const boostReady = boostCharge >= 1 && !boosting;
  const behind = !!player && snapshots.some(s => s.progress - player.progress > .005);
  return <div className="solo-race-hud">
    ...
    <div className="solo-ring-count" aria-label={`${player?.rings ?? 0} ChatGPT resets collected`}><span aria-hidden="true">↻</span><strong>{player?.rings ?? 0}</strong><small>RESETS</small></div>
    <div className="solo-position"><span>POSITION</span><strong>{player && !countdown ? racePlace(player, snapshots) : '—'}<small>/ {snapshots.length || 8}</small></strong><div><span className="live-dot"/> YOU · BLUE CREW</div></div>
    ...
    <div className="solo-hop"><div className={`solo-cue ${charge > .9 ? 'charged' : ''}`} data-kind={cue.kind}>{!countdown && cue.kind === 'turn-left' && <ArrowLeft size={16}/>} {!countdown && cue.kind === 'turn-right' && <ArrowRight size={16}/>} {countdown ? 'The hill is yours' : cue.label}</div><div className="solo-action-row"><button className={`hop-control ${charge > .06 ? 'is-charging' : ''}`} disabled={countdown || player?.finished} onPointerDown={event => { event.preventDefault(); event.currentTarget.setPointerCapture(event.pointerId); held.current = true; onHold(true); }} onPointerUp={() => { held.current = false; onHold(false); }} onPointerCancel={cancel} onLostPointerCapture={cancel} aria-label="Hold to charge, release to hop"><span className="charge-fill" style={{ transform: `scaleX(${charge})` }}/><kbd>SPACE</kbd><strong>{player?.finished ? 'Across the line!' : charge > .06 ? 'Release to fly' : 'Hold to charge'}</strong><Zap size={23}/></button><button type="button" className={`boost-control ${boosting ? 'is-boosting' : boostReady ? 'is-ready' : ''}`} disabled={countdown || !player || player.finished || player.recovering || !boostReady} onClick={event => { event.currentTarget.blur(); onBoost(); }} aria-label={boosting ? 'Boost active' : boostReady ? 'Boost your car (Z)' : 'Boost recharging'}><span className="boost-fill" style={{ transform: `scaleX(${boosting ? 1 : boostCharge})` }}/><kbd>Z</kbd><strong>{boosting ? 'BOOST!' : boostReady ? 'Boost' : `${Math.floor(boostCharge * 100)}%`}</strong></button></div><span className="solo-control-hint">{boosting ? 'KEEP STEERING · MAKE YOUR MOVE' : !boostReady && behind ? 'CATCH-UP · BOOST RECHARGES FASTER' : '← → STEER · SPACE / F HOP · TAP Z TO BOOST'}</span></div>
  </div>;
}
...
export default function DerbyUI(props: DerbyUIProps) {
  ...
  return <div className={`derby-ui stage-${stage} ${landing ? 'is-landing' : ''} ${props.phoneRoom ? 'phone-mode' : ''} ${solo ? 'solo-mode' : ''} ${localKeyboard ? 'local-keyboard-mode' : ''}`}>
    ...
    {showRules && <aside className="rules-popover"><h2>{props.phoneRoom ? "One button. All the glory." : "Build. Steer. Send it."}</h2><p>Pick a chassis and wheels. Your ride accelerates for you.</p><p><strong>Hold to charge. Release to hop.</strong><br/>{props.phoneRoom ? 'Use the big button on your phone.' : solo ? 'Arrow keys steer. Hold Space or F to charge; release to hop. Tap Z for a boost; it recharges faster when you fall behind. Or use the on-screen controls. Escape pauses.' : 'Player 1: A / D steer, F hops. Player 2: ← / → steer, J hops. Hold your hop key to charge, then release. You can also use each player’s on-screen controls. Escape pauses.'}</p>{solo && <label className="steering-preference"><input type="checkbox" checked={props.reverseArrows} onChange={event=>props.onReverseArrowsChange(event.target.checked)}/><span>Reverse arrow keys</span></label>}{solo && <p>Tap the camera button or press C to switch between Chase and Scenic views.</p>}<p>Three heats. Finish ahead to earn more points: {solo ? '10, 8, 6, 5, 4, 3, 2, 1' : '5, 3, 2, 1'}. Ties share the points.</p><Button className="small-action" onClick={() => setShowRules(false)}>Got it <Check size={17}/></Button></aside>}
    {landing && <section className="landing-screen landing-solo-track">
      <div className="landing-copy">
        <div className="landing-brand-row landing-brand-row-original"><SiliconBrand/><div className="landing-actions">{props.soundControls}<button className="icon-button" onClick={() => setShowRules(!showRules)} aria-label="How to play" aria-expanded={showRules}><HelpCircle size={18}/></button></div></div>
        <h1>Big ideas.<br/>Bad brakes.</h1>
        <p>Build your ride. Race seven rivals.<br/>Three laps around the bay.</p>
