...
const identical = runRace([DEFAULT_BUILDS[0], DEFAULT_BUILDS[0]], 'timed');
assert(identical.every((s) => s.finished), 'Identical builds should finish both lanes.');
assert(Math.abs(identical[0].finishTime! - identical[1].finishTime!) < 0.02, 'Identical builds and inputs should receive equivalent full-course results.');
...
console.log('Physics checks passed: gravity, charge cap, independent input/cancel, safe focus clear, airborne lock, four wheels, full-course lane fairness, recovery without progress gain, all 12 viable starters, and intentional timing over coasting/tapping/spam.');
