...
export function sessionCourse(phoneParty: boolean): PhysicsOptions {
  return phoneParty ? { course: 'classic', steeringEnabled: false }
    : { course: 'bay-or-bust', steeringEnabled: true, circuit: true, arcade: true, laps: 3 };
}

export function heatDeadline(mode: LocalMode, phoneParty: boolean, firstFinishAt: number | null): number {
  const limit = phoneParty ? 60 : 240;
...
export function keyboardSteering(mode: LocalMode, player: PlayerId, keys: ReadonlySet<string>, reverseArrows = false): number {
  ...
  return manualSteering(mode === 'solo' && reverseArrows ? -direction : direction);
}
