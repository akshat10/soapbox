...
export const ARCADE_DRIVE = { cruiseSpeed: 23, boostSpeed: 30, acceleration: 8, braking: 14, cornerAcceleration: 7, minimumSpeed: 8 } as const;
...
export function roadSpeedLimit(course: DerbyCourse, distance: number, speed: number, boosting: boolean): number {
  ...
  for (let ahead = 0; ahead <= Math.max(14, speed * 1.3); ahead += 4) {
    ...
    const crestSpeed = signatureJump ? 11.5 : Math.sqrt(10 / Math.max(.001,crest));
    const cornerSpeed = Math.max(ARCADE_DRIVE.minimumSpeed,Math.min(crestSpeed,Math.sqrt(radius*ARCADE_DRIVE.cornerAcceleration)));
